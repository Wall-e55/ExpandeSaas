...
<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
  <div>
    <h1 class="text-3xl font-bold text-[#2E338C]">Productos</h1>
    <p class="text-[#2E338C] text-sm">Listado de todos los productos</p>
  </div>
  <a href="{{ route('productos.create') }}" class="inline-flex items-center gap-2 px-6 py-2 rounded-lg bg-[#F2541B] text-white font-semibold shadow transition hover:bg-[#d9430f] focus:outline-none focus:ring-2 focus:ring-[#F2541B] focus:ring-offset-2">
  ...
  <table class="min-w-full divide-y divide-[#2E338C]/10">
    ...
    <tbody class="divide-y divide-[#2E338C]/10">
      ...
      <tr class="hover:bg-[#F2541B]/10 transition">
        ...
        <td class="px-4 py-2 font-semibold text-[#2E338C]">{{ $producto->nombre }}</td>
        <td class="px-4 py-2">${{ number_format($producto->precio, 2) }}</td>
        <td class="px-4 py-2 flex gap-2">
          <a href="{{ route('productos.edit', $producto) }}" class="px-3 py-1 rounded bg-[#2E338C] text-white text-xs font-medium hover:bg-[#23277a] transition">Editar</a>
