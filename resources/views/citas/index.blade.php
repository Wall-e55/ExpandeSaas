...
<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
  <div>
    <h1 class="text-3xl font-bold text-[#2E338C]">Citas</h1>
    <p class="text-[#2E338C] text-sm">Listado de todas las citas programadas</p>
  </div>
  <a href="{{ route('citas.create') }}" class="inline-flex items-center gap-2 px-6 py-2 rounded-lg bg-[#F2541B] text-white font-semibold shadow transition hover:bg-[#d9430f] focus:outline-none focus:ring-2 focus:ring-[#F2541B] focus:ring-offset-2">
  ...
  <table class="min-w-full divide-y divide-[#2E338C]/10">
    <thead class="bg-[#2E338C] text-white">
      <tr>
        ...
        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Motivo</th>
    ...
    <tbody class="divide-y divide-[#2E338C]/10">
      ...
      <tr class="hover:bg-[#F2541B]/10 transition">
        <td class="px-4 py-2 font-semibold text-[#2E338C]">{{ $cita->cliente->nombre ?? '-' }}</td>
        <td class="px-4 py-2">{{ \Carbon\Carbon::parse($cita->fecha_hora)->format('d/m/Y H:i') }}</td>
        <td class="px-4 py-2">{{ $cita->motivo }}</td>
        <td class="px-4 py-2">{{ $cita->estado }}</td>
        <td class="px-4 py-2 flex gap-2">
          <a href="{{ route('citas.edit', $cita) }}" class="px-3 py-1 rounded bg-[#2E338C] text-white text-xs font-medium hover:bg-[#23277a] transition">Editar</a>
