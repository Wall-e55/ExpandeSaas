...
<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
  ...
  <a href="{{ route('reportes.create') }}" class="inline-flex items-center gap-2 px-6 py-2 rounded-lg bg-[#F2541B] text-white font-semibold shadow transition hover:bg-[#d9430f] focus:outline-none focus:ring-2 focus:ring-[#F2541B] focus:ring-offset-2">
  ...
  <table class="min-w-full divide-y divide-[#2E338C]/10">
    ...
    <tbody class="divide-y divide-[#2E338C]/10">
      ...
      <tr class="hover:bg-[#F2541B]/10 transition">
        ...
        <td class="px-4 py-2 font-semibold text-[#2E338C]">{{ $reporte->titulo }}</td>
        <td class="px-4 py-2">{{ $reporte->descripcion }}</td>
        <td class="px-4 py-2 flex gap-2">
          <a href="{{ route('reportes.edit', $reporte) }}" class="px-3 py-1 rounded bg-[#2E338C] text-white text-xs font-medium hover:bg-[#23277a] transition">Editar</a>
