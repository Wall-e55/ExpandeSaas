...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow p-8 mt-8">
    <h1 class="text-2xl font-bold text-[#2E338C] mb-6">Editar Reporte</h1>
    <form action="{{ route('reportes.update', $reporte) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        <div>
            <label for="titulo" class="block text-sm font-medium text-[#2E338C] mb-1">Título</label>
            <input type="text" name="titulo" value="{{ old('titulo', $reporte->titulo) }}" class="w-full rounded-lg border border-[#2E338C]/20 focus:border-[#2E338C] focus:ring-2 focus:ring-[#2E338C]/30 px-4 py-2" required>
        </div>
        <div>
            <label for="descripcion" class="block text-sm font-medium text-[#2E338C] mb-1">Descripción</label>
            <textarea name="descripcion" class="w-full rounded-lg border border-[#2E338C]/20 focus:border-[#2E338C] focus:ring-2 focus:ring-[#2E338C]/30 px-4 py-2" required>{{ old('descripcion', $reporte->descripcion) }}</textarea>
        </div>
